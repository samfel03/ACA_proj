...
#endif
#include "app.h"
//#include "dev_options.h", stopwatch.h, init.h, init.cpp not used
// ------------------- FPGA LIBRARIES
 ...
 */
{
    ...
    while (error >= tol)
    {
        ...
        for (int i = 0; i < NPOSLAND; i++)
        {
            real rhs = 0;
            for (int j = 0; j < NPOSLAND; j++)
                rhs += trmult_reduced[i*NPOSLAND + j] * integral[j];
 ...
 */
{
    ...
    #ifdef SERIAL_CPU_MODE
    loop_uhat(TOLL_MODEL_INITIAL_PERIOD,env->trmult_reduced,FL,env->uhat_f_exp,uhat_i,FR,env->exp_uhatR);
    #elif FPGA_MODE
    host_to_fpga(TOLL_MODEL_INITIAL_PERIOD,env->trmult_reduced,FL,env->uhat_f_exp,uhat_i,FR,env->exp_uhatR);
    #endif
 ...
 */
{
    ...
    #elif FPGA_MODE
    host_to_fpga(TOLL_M2_INNER,env->trmult_reduced,ML,L_exp,m2_i,MR,R_exp);
    #endif
    ...
    for (size_t t = 0; t < T_FORWARD; t++)
    {
        ...
        while (error_realgdp >= FW_tol_realgdp)
        {
            ...
            while (error_e >= FW_tol_e)
            {
                ...
                #elif FPGA_MODE
                host_to_fpga(BW_tol,env->trmult_reduced,FL,env->uhat_f_exp,uhat_i,FR, env->exp_uhatR);
                #endif
        ...
        */
        ...
        while (error_realgdp >= BW_tol_realgdp)
        {
            ...
            while (error_e >= BW_tol_e)
            {
                ...
                #elif FPGA_MODE
                host_to_fpga(BW_tol,env->trmult_reduced,BL,env->uhat_f_exp,uhat_i,BR, env->exp_uhatR);
                #endif
                // Preallocate loop that does not depend on the next while but for ltilde
                real l_vec[NPOSLAND];
                real sum_u0m2 = 0.;
