...
#include "projectTypes.h"  // #typedef double real;
 ...
 */
{
    Eigen::VectorXd integral(NPOSLAND);
    Eigen::VectorXd uhat_f(NPOSLAND);
    Eigen::VectorXd rhs(NPOSLAND);
    real error = tol + 1.;
    int iter = 0;

    // Map the array pointers to an 'Eigen' vector and matrix types
    Eigen::Map<const Eigen::MatrixXd> trmult_reduced_map(trmult_reduced, NPOSLAND, NPOSLAND);
    Eigen::Map<const Eigen::VectorXd> L_map(L, NPOSLAND);
    Eigen::Map<const Eigen::VectorXd> R_map(R, NPOSLAND);
    Eigen::Map<Eigen::VectorXd> uhat_i_map(uhat_i, NPOSLAND);

    while (error >= tol)
    {
        // Compute accuracy and update the guess
        integral = R_map.array() * uhat_i_map.array().pow(R_exp);

        rhs = trmult_reduced_map * integral;

        uhat_f = L_map.array() * rhs.array().pow(L_exp);

        Eigen::VectorXd deviation = uhat_i_map - uhat_f;
        error = deviation.squaredNorm();
 ...
 */
{
    ...
    for (size_t t = 0; t < T_BACK - 1; t++) // Might need to be T_BACK-1
    {
        ...
        */
        ...
        while (error_realgdp >= BW_tol_realgdp)
        {
            ...
            while (error_e >= BW_tol_e)
            {
                ...
                loop_uhat_sparse(env, BW_tol, BL, env->uhat_f_exp, uhat_i, BR, env->exp_uhatR);
