"""
This script can be used to check the absolute difference between the results generated from fpga (hw_checks)
to reference values generated from c code (checks). 

Required installations to use this script:
cd /tmp/
...
path = os.getcwd()
print(path)
cpu_folder = "results/seq_cpu"
fpga_folder = "results/fpga"

file_names_fpga = ["V_fpga_nk8_d0_k0.txt"] 
file_names_cpu = ["V_serial-cpu_nk8.txt"]   # ["kcross_serial-cpu_nKM4-nk100.txt", "kcross_serial-cpu_nKM4-nk200.txt"]
for i in range(len(file_names_fpga)):
    sw_file = (os.path.join(path, cpu_folder, file_names_cpu[i]))
    hw_file = (os.path.join(path, fpga_folder, file_names_fpga[i]))
    ...
    # Results
    if(equal):
        print("\n{} Test Passed".format(file_names_fpga[i]))
    else:
        print("\n{} Test Failed: Results mismatch".format(file_names_fpga[i]))

    print("{} Absolute maximum difference is: {}".format(file_names_fpga[i], max_diff))
    print("{} Mean of the difference is: {}\n".format(file_names_fpga[i], avg_diff))
    

'''
ref_folder = "results/matlab"                       #folder containing reference values
fpga_folder = "results/fpga/fixed/final_values"     #folder with output results generated from fpga
cpu_folder = "results/seq_cpu/double/final_values"     #folder with output results generated from fpga
