...
#define INPUTPATH "/scratch/summit/alpe9373/CCE-C/auxiliary/"
...
#define IPOSLAND(i, j) (i * NPOSLAND + j)		// location i-location j cost
#define IPOSLANDTPLUS1(i, t) (t * NPOSLAND + i) // time t-location i
#define IPOSLANDT(i, t) (t * NPOSLAND + i)		// time t-location i
#define IPROVA(i, j) (j * 4 + i)				// i*NCOL_MATLAB + j, row index: i, col index (j) 
...
int main()
{
	...
	// free_all(&env);
	char variablename[250] = {'\0'};
	real *A = (real *)calloc(8, sizeof(real));
	// ---------------------------------------------------------------------------
	memset(variablename, '\0', sizeof(variablename));
	strcpy(variablename, "A");
	read_binary(A, variablename,8); // SET VARIABLE
	for (int i=0;i<8;i++)
		printf("%lf \n",A[i]);
	printf("\n");
	printf("%lf %lf \n %lf %lf\n %lf %lf \n %lf %lf\n",A[IPROVA(0,0)],
