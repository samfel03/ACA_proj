...
extern "C" {
#include "../library/rt_nonfinite.h"
#include "../library/rtwtypes.h"
#include "../library/svd.h"
...
// const string PROJECT_PATH = "/Users/alessandroperi/|Dropbox/Projects/CCE-FPGA/CCE-C/C/";
// const string PROJECT_PATH = "E:/Code/IS/eco_data/CCE-C/C/";
// const string PROJECT_PATH = "/mnt/e/Code/IS/eco_data/CCE-C/C/";
// const string PROJECT_PATH = "/mnt/e/Code/IS/test/CU-Spur-Sean/xcode_upmem/CCE-C/CCE-C/C/";
const string PROJECT_PATH = "/mnt/e/Code/IS/test/CU-Spur-Sean/fol/C/";

// #define TRMULTPATH (PROJECT_PATH + "input/trmult_reduced.bin").c_str()
const string TRMULTPATH = PROJECT_PATH + "input/trmult_reduced.bin";

const size_t size_of_trmult_reduced = sizeof(real);
const real TRMULT_REDUCED_THRESHOLD = 1e-14;
#define BLOCK_SIZE_UHAT 8      // 8*17048
#define BLOCK_SIZE_READ 136384 // 8*17048
#define INPUTPATH (PROJECT_PATH + "input/").c_str()
#define PRINTPATH (PROJECT_PATH + "output/").c_str()
#ifdef DEBUG_MODE
#define DEBUGPATH (PROJECT_PATH + "input/debug/").c_str()
#endif
#define N_MODEL 1
...
// #define IPOSLAND(i, j) ((i) * trmult_reduced_cols + (j))Z
unsigned int input_size_dpu_8bytes;
unsigned int integral_input_size_dpu_8bytes;
...
size_t read_block(real *X_var, size_t offset) {
  // Open the file
  FILE *fp = fopen(TRMULTPATH.c_str(), "rb"); // // rb: read binary
...
void write_binary(real *X_var, char *variablename, int dim_n) {
  cout << "Writing begins for: " << variablename << endl;
  FILE *fp;
  char filename[250] = {'\0'};
  memset(filename, '\0', sizeof(filename));
  cout << "Memset begins for: " << variablename << endl;

  strcpy(filename, PRINTPATH);
  cout << "strcpy printpath for: " << variablename << endl;
  strcat(filename, variablename);
  cout << "strcat filename for: " << variablename << endl;
  strcat(filename, ".bin");
  cout << "strcat filename for filename.bin: " << variablename << endl;
  fprintf(stderr, "Opening in write_binary the file: %s\n", filename);
  ...
  // Write binary
  cout << "fwrite start: " << variablename << endl;
  fwrite(X_var, sizeof(X_var[0]), dim_n, fp);
  cout << "fwrite end: " << variablename << endl;

  fclose(fp);
  fp = NULL;
  cout << "Writing complete for: " << variablename << endl;
  return;
}

// CASE:
void read_chunk(real *X_var, size_t start, size_t end, const char *data) {
  for (size_t i = start; i < end; ++i) {
    X_var[i] = reinterpret_cast<const real *>(data)[i];
  }
}
...
long long get_total(time_point<high_resolution_clock> start_time, time_point<high_resolution_clock> end_time) {
...
void read_binary(real *X_var, char *variablename, int dim_n) {
  ...
  cout << "Starting read_binary for: " << variablename << endl;
  FILE *fp;
  char filename[250] = {'\0'};

  // cout << "Starting memset for: " << variablename << endl;
  memset(filename, '\0', sizeof(filename));

  // cout << "Strcpy inputpath for filename: " << variablename << endl;
  strcpy(filename, INPUTPATH);

  // cout << "Strcat for: " << variablename << endl;
  strcat(filename, variablename);

  // cout << "Strcat for binary file: " << variablename << endl;
  strcat(filename, ".bin");

  fprintf(stderr, "Opening in read_binary the file: %s\n", filename);

  // ----------------------------------------------------------------

  // Open the file
  // fp = fopen(filename, "rb"); // // rb: read binary
  // cout << "File opened: " << variablename << endl;
  ...
  if (fd == -1) {
    cerr << "\n1.1. FAIL: Cannot read the variable " << variablename << endl;
    exit(EXIT_FAILURE);
  }
  // ----------------------------------------------------------------
  ...
  fprintf(stderr, "Variable %s is of size: %lu bytes\n", variablename, SIZE_var);

  // %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  // --------------------------------------------------------------------

  void *map = mmap(0, SIZE_var, PROT_READ, MAP_SHARED, fd, 0);
  if (map == MAP_FAILED) {
    close(fd);
  ...
  // cout << "Read binary start for: " << variablename << endl;
  size_t num_threads = thread::hardware_concurrency();
  // cout << "Total Available threads: " << num_threads << endl;
  vector<thread> threads;
  size_t chunk_size = dim_n / num_threads;

  for (size_t i = 0; i < num_threads; i++) {
    size_t start = i * chunk_size;
    size_t end = (i == num_threads - 1) ? dim_n : start + chunk_size;
    threads.emplace_back(read_chunk, X_var, start, end, static_cast<const char *>(map));
  }
  ...
  // cout << "Read binary end for: " << variablename << endl;
  munmap(map, SIZE_var);
  close(fd);
  ...
  auto duration = get_total(start_time, end_time);

  // Output the duration
  cout << "(Threads: " << num_threads << "): Time taken to complete read_binary for " << variablename << ": " << duration << " milliseconds" << endl;
...
void debug_NLLS_temporary_read(env_t *env) {
  ...
  strcpy(variablename, "coeff_pop_i");
  read_binary_debug(env->coeff_pop_i, variablename, LENGTH_COEFF_POP);
}
void debug_backward_climate_part_1(env_t *env) {
  ...
  // ---------------------------------------------------------------------------
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "bwd_l0_model");
  write_binary(env->l0_model, variablename, NPOSLAND);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "bwd_realgdp0_model");
  write_binary(env->realgdp0_model, variablename, NPOSLAND);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "bwd_weight_pop_matrix");
  write_binary(env->weight_pop_matrix, variablename, NPOSLAND);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "bwd_b0T");
  write_binary(&env->b0T, variablename, 1);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "bwd_bw");
  write_binary(&env->bw, variablename, 1);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "bwd_l20_model");
  write_binary(env->l20_model, variablename, NPOSLAND);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "bwd_realgdp20_model");
  write_binary(env->realgdp20_model, variablename, NPOSLAND);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "bwd_temp20_local");
  write_binary(env->temp20_local, variablename, NPOSLAND);
}
...
void debug_backward_climate_output(sym_t *sym_) {
  ...
  // ---------------------------------------------------------------------------
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "bwd_sym_l_b");
  write_binary(sym_->l_b, variablename, NPOSLAND * T_BACK);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "bwd_sym_realgdp_b");
  write_binary(sym_->realgdp_b, variablename, NPOSLAND * T_BACK);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "bwd_sym_emiCO2_b");
  write_binary(sym_->emiCO2_b, variablename, NPOSLAND * T_BACK);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "bwd_sym_clean_b");
  write_binary(sym_->clean_b, variablename, NPOSLAND * T_BACK);
}
void debug_backward_climate_temporary_read(sym_t *sym_) {
  ...
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "l_b");
  read_binary_debug(sym_->l_b, variablename, alloc_size);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "realgdp_b");
  read_binary_debug(sym_->realgdp_b, variablename, alloc_size);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "emiCO2_b");
  read_binary_debug(sym_->emiCO2_b, variablename, alloc_size);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "clean_b");
  read_binary_debug(sym_->clean_b, variablename, alloc_size);
}
void debug_forward_climate_temporary_read(sym_t *sym_) {
  ...
  read_binary_debug(sym_->l, variablename, alloc_size);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "prod");
  read_binary_debug(sym_->prod, variablename, alloc_size);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "realgdp");
  read_binary_debug(sym_->realgdp, variablename, alloc_size);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "amen");
  read_binary_debug(sym_->amen, variablename, alloc_size);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "temp_local");
  read_binary_debug(sym_->temp_local, variablename, alloc_size);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "price_emi");
  read_binary_debug(sym_->price_emi, variablename, alloc_size);
  memset(variablename, '\0', sizeof(variablename));
  strcpy(variablename, "price_clean");
  read_binary_debug(sym_->price_clean, variablename, alloc_size);
}
...
real natal_fct_logrealgdp(env_t *env, const real logrealgdp) {
  ...
  real x = (env->logi_b0y_fct_0 + (env->logi_b2y_fct_3 - env->logi_b0y_fct_0) * exp(-env->logi_b1y_fct_1 * square)) * (logrealgdp < env->logi_bsy_fct_4) +
           (env->logi_b2y_fct_3 * exp(env->minus_exp_2 * square)) * (logrealgdp >= env->logi_bsy_fct_4);
  return x;
}
...
void loop_uhat(const real tol, const real *trmult_reduced, const real *L, const real L_exp, real *uhat_i, const real *R, const real R_exp) {
  /* input:
    - tol: level of tolerance, scalar, real
    - trmult_reduced: 17048x17048, real
    - L,R: 17048x1, real
  ...
  */
  ...
  int accumulator_index[accumulator_index_count], i = 0;
  // accumulator_index[0] = integral_rows;
  for (i = 0; i < accumulator_index_count; i++) {
    accumulator_index[i] = integral_rows * (i + 1);
  }

  // loop_uhat_compute(tol, trmult_reduced, L, L_exp, uhat_i, R, R_exp);
  real integral[NPOSLAND];
  real error = tol + 1.;
  int iter = 0;
 ...
 */
{
  ...
  real sum = 0.;
  int i = 0;
  stockCO2_layers[i] = S0 + env__a0 * emi;
  sum += stockCO2_layers[i];
  i++;
  stockCO2_layers[i] = exp(-1. / env__b1) * S1 + env__a1 * emi;
  sum += stockCO2_layers[i];
  i++;
  stockCO2_layers[i] = exp(-1. / env__b2) * S2 + env__a2 * emi;
  sum += stockCO2_layers[i];
  i++;
  stockCO2_layers[i] = exp(-1. / env__b3) * S3 + env__a3 * emi;
  sum += stockCO2_layers[i];
  real forc;
...
#ifdef DEBUG_MODE
        printf("t = %u; error_realgdp = %.9lf; error_e = %.9lf;\n", t, error_realgdp, error_e);
#endif
...
string generateUniqueFilename(const string &baseName, const string &extension) {
  string filename = baseName + extension;
  int count = 1;

  // Check if the file exists using <fstream>
  while (ifstream(filename)) {
    filename = baseName + "_" + std::to_string(count) + extension;
    count++;
  }
  return filename;
}
...
int main(int argc, char **argv) {
  string baseName = "output"; // Base name of the file
  string extension = ".txt";  // File extension

  // Generate a unique filename
  string uniqueFilename = generateUniqueFilename(baseName, extension);

  FILE *file = freopen(uniqueFilename.c_str(), "w", stdout);
  if (!file) {
    std::cerr << "Error opening file for output." << std::endl;
    return 1;
  }

  // ------------------------------------------ Including DPU Pre-Processing
  ...
  input_size_dpu_8bytes = set_matrix_dpu_input_params(trmult_reduced_rows, trmult_reduced_cols, 1);

  // DONE: QUANTITY: Integral initializer (Element based calculation)
  integral_input_size_dpu_8bytes = set_vector_dpu_input_params(integral_rows, integral_cols, 1);
