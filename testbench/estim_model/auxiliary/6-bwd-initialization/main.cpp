...
real natal_fct_logrealgdp(env_t *env,real logrealgdp){
    // Construct income component of natality rate function, equation (23)
    real square = (logrealgdp-logi_bsy_fct(env->coeff_pop_i[4]))*(logrealgdp-logi_bsy_fct(env->coeff_pop_i[4]));
    real x =(logi_b0y_fct(env->coeff_pop_i[0]) +
             (logi_b2y_fct(env->coeff_pop_i[3])-logi_b0y_fct(env->coeff_pop_i[0]))*exp(-logi_b1y_fct(env->coeff_pop_i[1])*square)
             )*(logrealgdp<logi_bsy_fct(env->coeff_pop_i[4])) +
            (logi_b2y_fct(env->coeff_pop_i[3])*exp(-exp(env->coeff_pop_i[2])*square)
            )*(logrealgdp>=logi_bsy_fct(env->coeff_pop_i[4]));
    return x;
}
void natal_fct(env_t *env,real *logrealgdp,real logrealgdp_w,int t,real *natal_fct_val){
    ...
    for (int i = 0; i < NPOSLAND; i++){
        ...
        real square = (env->temp0_local[i]-logi_bsT_fct(env->coeff_pop_i[7]))*(env->temp0_local[i]-logi_bsT_fct(env->coeff_pop_i[7]));
        sum_coef5pop0    +=exp(-exp(env->coeff_pop_i[5])*square)*pop0_sh;
        sum_nat_gdp0pop0+=natal_fct_logrealgdp(env,log(env->realgdp0_model[i]))*pop0_sh;
        
        real l20H_i= env->l20_model[i]*env->H[i];
        sum_l20H        +=l20H_i;
        real pop20_sh = env->l20_model[i]*env->H[i];///sum_l20H;
        real square20=(env->temp20_local[i]-logi_bsT_fct(env->coeff_pop_i[7]))*(env->temp20_local[i]-logi_bsT_fct(env->coeff_pop_i[7]));
        sum_bw_1        += logi_b2T_fct(env->coeff_pop_i[6])*exp(-exp(env->coeff_pop_i[5])*square20)*pop20_sh;
        sum_bw_2        += natal_fct_logrealgdp(env,log(env->realgdp20_model[i]))*pop20_sh;
        logrealgdp20_w+=env->realgdp20_model[i]*l20H_i;
    }
    sum_bw_1=sum_bw_1/sum_l20H;
    sum_bw_2=sum_bw_2/sum_l20H;
    logrealgdp20_w=log(logrealgdp20_w/sum_l20H);
    // Construct b0T, equation (31)
    real b0T = 2.0*env->natal0 - logi_b2T_fct(env->coeff_pop_i[6])*sum_coef5pop0 - 2.0*sum_nat_gdp0pop0;
    ...
    real natal_fct_temp_denom = (1.+exp(bw*(logrealgdp_w-logrealgdp0_w)));        // current logrealgdp_w
    // Construct numerator of temperature component of natality rate function, equation (24)
    real natal_coeff_6 = logi_b2T_fct(env->coeff_pop_i[6]);
    real natal_coeff_5 = -exp(env->coeff_pop_i[5]);
    for (int i = 0; i < NPOSLAND; i++){
        int is = IPOSLANDT(t, i);
        real square = (env->temp_past[is]-logi_bsT_fct(env->coeff_pop_i[7]))*(env->temp_past[is]-logi_bsT_fct(env->coeff_pop_i[7]));
        real natal_fct_temp_num = b0T + natal_coeff_6*exp(natal_coeff_5*square);
        // Construct natality functions
        natal_fct_val[i] = natal_fct_temp_num / natal_fct_temp_denom + natal_fct_logrealgdp(env,logrealgdp[i]);
        //complex function???? real(natal_fct_temp_num / natal_fct_temp_denom + natal_fct_logrealgdp(env,logrealgdp)); ****** DEVELOP ******
    }
}
void loop_uhat(real tol, real *trmult_reduced, real *integral, real *L, real L_exp,
...
void climate_theta(size_t T, const real min, const real max, const real center, const real steep, real *past, real *theta_temp)
{
    ...
    for (size_t t = 0; t < T; t++)
        for (int i = 0; i < NPOSLAND; i++)
            theta_temp[IPOSLANDT(t, i)] = min + diff / (1. + exp(steep * (past[IPOSLANDT(t, i)] - center)));
...
void climate_read_parameters(env_t *env)
{
    // Read parameters
    /*
        ...
        while (error_realgdp >= BW_tol_realgdp)
        {
            ...
            while (error_e >= BW_tol_e)
            {
                ...
                for (int i = 0; i < NPOSLAND; i++)
                {
                    ...
                    price_energy[i]= pow( pow(env->fossil_share,env__eps)*pow(price_fossil,one_minus_epsilon)
                                           + pow(1.-env->fossil_share,env__eps)*pow(price_clean,one_minus_epsilon)
                                         ,one_over_one_minus_epsilon);
                    BL[i] = BL_aux[i]* pow(price_energy[i], (env->BL_price_energy0_exp)); // outside the integral
                    BR[i] = BR_aux[i]* pow(price_energy[i], (env->BR_price_energy0_exp)); // inside the integral
                    integral[i] = BR[i] * pow(uhat_i[i], env->BW_exp_uhatR);
                    // printf("%lf %lf %lf %lf\n",FL[i],FR[i],uhat_i[i],integral[i]);
                }
                // Iterate uhat, equation (88)
                loop_uhat(BW_tol, env->trmult_reduced, integral,BL, env->BW_uhat_f_exp,
                          uhat_i, BR, env->BW_exp_uhatR);
                ...
                while (error_l >= BW_tol_l)
                {
                    ...
                    */
                    real u_i_scaler = const_phi_to_minus_gamma12 *
                                      ...
                                      pow(ltilde/sum_u0m2,env->natural/env__theta);        // this third part could be preallocated // ****** DEVELOP ******
                    
                    real *logrealgdp = (real *)malloc(NPOSLAND * sizeof(real));
                    realgdp_w =0.;
                    for (int i = 0; i < NPOSLAND; i++){
                        ...
                        // Calculate real GDP per capita, equation (4)
                        logrealgdp[i]= log(real_gdp_i);
                        // Real GDP
                        uint32_t is = IPOSLANDT(t, i);
                        sym_->realgdp_b[is]=real_gdp_i;
                    }
                    // Net_births
                    real logrealgdp_w= log(realgdp_w);
                    real *net_births = (real *)malloc(NPOSLAND * sizeof(real));
                    natal_fct(env,logrealgdp,logrealgdp_w,t,net_births);                            // ****** DEVELOP: MAY drop logrealgdp and net_births ******
                    // Update global population
                    real ltilde1=0.;
                    for (int i = 0; i < NPOSLAND; i++){
                        ltilde1+=net_births[i]*l_vec[i]*env->H[i];
                        // Return:
                        uint32_t is = IPOSLANDT(t, i);
                ...
                for (int i = 0; i < NPOSLAND; i++){
                    ...
                    real emiCO2_ff=const_energy_l_over_price_energy*pow(env->fossil_share*price_energy_real/price_fossil,env__eps);
                    real clean =const_energy_l_over_price_energy*pow((1.-env->fossil_share)*price_energy_real/price_clean,env__eps);
...
void model_initial_period(env_t *env)
{
    ...
    loop_uhat(TOLL_MODEL_INITIAL_PERIOD, env->trmult_reduced, integral, FL, env->uhat_f_exp,
              uhat_i, FR, env->exp_uhatR);
    /*
...
void init_all(env_t *env) //, input_t *in, vars_t *vars)
{
    ...
    env->FL_H_m2_left = (real *)calloc(NPOSLAND, sizeof(real)); // model_initial_period
    env->FR_H_m2_left = (real *)calloc(NPOSLAND, sizeof(real)); // model_initial_period
    ...
    env->BL_H_m2_left = (real *)calloc(NPOSLAND, sizeof(real)); // backward_climate
    env->BR_H_m2_left = (real *)calloc(NPOSLAND, sizeof(real)); // backward_climate
    ...
    // Initialize Auxiliary to model_init_period, backward_climate
    init_model(env);
...
void free_all(env_t *env)
{
    ...
    free(env->BL_H_m2_left);
    free(env->BR_H_m2_left);
