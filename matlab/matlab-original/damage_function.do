...
clear
import delimited "Data/derived/amen_prod.csv", clear 
...
compress
save "Data/derived/amen_prod.dta", replace

clear
import delimited "Data/subcountry_EU.csv", clear 
compress
save "Data/derived/subcountry_EU.dta", replace
...
clear 
import delimited "Data/HDI_GDPpc.csv", clear 
...
merge 1:1 lat lon year using "Data/derived/amen_prod.dta"
qui drop _merge
...
clear
import delimited "Data/geo_data.csv", clear 
capture rename ïlat lat

compress
save "Data/derived/geo_data.dta", replace
...
clear
import delimited "Data/temp_data.csv", clear 
...
qui merge m:1 lat lon using "Data/derived/geo_data.dta" 
qui drop _merge

* Merge amenities and productivity
qui merge 1:1 year lat lon using "Data/derived/amen_prod.dta"
qui drop if _merge != 3
qui drop _merge

* Merge amenities and productivity
qui merge 1:1 year lat lon using "Data/derived/subcountry_EU.dta"
qui drop if _merge != 3
qui drop _merge
...
compress 
save "Data/derived/data_damage_fct.dta", replace
...
foreach y of numlist 1 10 {
	foreach b of numlist 20 50 {
		...
		foreach x of local month_name {
			...
			use "Data/derived/data_damage_fct.dta", clear
			qui _pctile temp_`y'y_`x', percentiles(`ptiles')
			matrix temp_pctile = [`rptiles']'

			* Store results from regression as data
			qui drop _all
			...
			qui compress
			qui save "Data/derived/temp_`y'y_`x'_pctile`b'.dta", replace
		}
...
foreach f of local fund_name {
	foreach s of local specif_name {
		...
		* Transform results to data
		...
		qui merge 1:1 bin using "Data/derived/temp_`y'y`m'_pctile`b'.dta"
		qui drop _merge
		...
		* Estimate logistic smoothing
		...
		compress
		save "Data/derived/`f'_point_`y'y`m'_`b'b_550d.dta", replace
		...
		compress
		save "Data/derived/`f'_coeff_`y'y`m'_`b'b_550d.dta", replace

		export delimited using  "Data/derived/`f'_coeff_`y'y`m'_`b'b_550d.csv", replace

		use "Data/derived/`f'_point_`y'y`m'_`b'b_550d.dta", clear
...
foreach f of local fund_name {
	...
	* Save errors
	foreach e of local error_name {
		...
		compress
		save "Data/derived/`f'_point_`y'y`m'_`b'b_`e'.dta", replace
...
foreach f of local fund_name {
	...
	* Baseline specification
	...
	foreach e of local error_name {
		qui merge 1:1 bin using "Data/derived/`f'_point_`y'y`m'_`b'b_`e'.dta", nogenerate
		qui replace se = round(se,.0001)
		format se %8.4f
