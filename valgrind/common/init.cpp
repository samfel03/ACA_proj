...
void read_small_idx_1D(small_idx_t *variable, char *variablename)
{
	FILE *fp;
	char filename[250] = {'\0'};
	memset(filename, '\0', sizeof(filename));
	...
	int i = 0;
	while (fscanf(fp, "%hd", &variable[i]) == 1)
		i = i + 1;
	fprintf(stderr, "Total Number of Elements: %d\n", i);
...
void printf_small_idx_1D(int dim_1, small_idx_t *A)
{
	int i_1;
	for (i_1 = 0; i_1 < dim_1; i_1++)
	{
		fprintf(stderr, "%hd ", A[i_1]);
	}
}
...
{
	// Auxiliary Variables
	real *amen_util0 = (real *) malloc(NPOSLAND*sizeof(real));
	// Free
	free(amen_util0);
...
void read_binary(real *X_var, char *variablename, int dim_n){
  ...
  // Variable size
  size_t SIZE_var = (size_t) (dim_n * sizeof(real)); // Number of bytes
  fprintf(stderr, "Variable %s has dimensions %d and size %lu bytes\n",variablename,dim_n,SIZE_var); //%lu if it was size_t
  // Read binary
  ...
  int index=12;
  fprintf(stderr, "Variable %s at %d is %lf\n",variablename,index,X_var[index]); //%lu if it was size_t
  fclose(fp);
  fp = NULL;
  return;
}
...
{
	// Prellocate
	env->H = (real *)malloc(NPOSLAND * sizeof(real));
	env->prod0 = (real *)malloc(NPOSLAND * sizeof(real)); // productivities in 2000
	env->pop5_dens = (real *)malloc(NPOSLAND * sizeof(real));
	env->u0 = (real *)malloc(NPOSLAND * sizeof(real));						  // utility in 2000
	env->a_norm = (real *)malloc(NPOSLAND * sizeof(real));					  // amenities in 2000
	env->trmult_reduced = (real *)malloc(NPOSLAND * NPOSLAND * sizeof(real)); // Bilateral trade cost on Earth cells at t=0 only
	env->C_vect = (small_idx_t *)malloc(NPOSLAND * sizeof(small_idx_t));	  // national demarcations with H0>0
	env->emi_no_ff = (real *)malloc(N2600 * sizeof(real));					  // Global CO2 emissions from non fossil fuels from 2000 to 2600, by IPCC					// utility in 2000
	env->price_energy0 = (real *)malloc(NPOSLAND * sizeof(real));
	env->zeta_clean0 = (real *)malloc(NPOSLAND * sizeof(real));
	env->zeta_fossil0 = (real *)malloc(NPOSLAND * sizeof(real));
	env->forc_noCO2 = (real *)malloc(N2600_2 * sizeof(real)); // Read non CO2 radiative forcing, 601x1
	env->temp_past = (real *)malloc(NPOSLAND * T_BACK_AUX * sizeof(real));
	env->Delta_temp = (real *)malloc(NPOSLAND * T_BACK * sizeof(real));
	env->scaler_temp = (real *)malloc(NPOSLAND * sizeof(real));
	env->theta_amen_scen = (real *)malloc(4 * 9 * sizeof(real));
	env->theta_prod_scen = (real *)malloc(4 * 9 * sizeof(real));
	env->natal_data = (real *)malloc(NDATA * sizeof(real));
	...
	// ---------------------------------------------------------------------------
	memset(variablename, '\0', sizeof(variablename));
	strcpy(variablename, "temp_past");read_binary(env->temp_past, variablename,NPOSLAND* T_BACK_AUX); // SET VARIABLE
	// ---------------------------------------------------------------------------
	memset(variablename, '\0', sizeof(variablename));
	...
	// ---------------------------------------------------------------------------
	memset(variablename, '\0', sizeof(variablename));
	strcpy(variablename, "theta_prod_scen");read_real_1D(env->theta_prod_scen, variablename); // ************* VARIABLE
	printf_real_1D(NPOSLAND, env->theta_prod_scen);printf("\n");	// Check: Print Variable
	*/
