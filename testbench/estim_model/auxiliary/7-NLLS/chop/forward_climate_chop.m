...
realincome = zeros(n,T);                % AUXILIARY - not needed
price_energy_tilde = zeros(n,T);        % AUXILIARY - not needed
varphi = zeros(n,T);
cumCO2_ff = zeros(T,1);
emiCO2_ff_abat = zeros(n,T);
emiCO2_total = zeros(T,1);
stockCO2_layers = zeros(4,T);
forc = zeros(T,1);                      % AUXILIARY - not needed
temp_layers = zeros(2,T);               % AUXILIARY - not needed
...
if ind_exog == 0    % endogenous CO2 emissions, temperature and population
    ...
    if ind_clim == 1
        data_Warm = load([datafolder,'/derived/results_forward_Warm_' name_dam_vect{ind_dam} '.mat']);
        emiCO2_ff = data_Warm.emiCO2_ff_Warm;
        temp_global = data_Warm.temp_global_Warm;
        lbar_time = sum((data_Warm.l_Warm).*repmat(H,1,T),1);
    else
        data_noWarm = load([datafolder,'/derived/results_forward_noWarm.mat']);
