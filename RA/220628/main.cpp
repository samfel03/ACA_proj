...
#define INPUTPATH "C:\\Users\\Joongsan\\source\\repos\\AleP83\\CCE-C\\testbench\\estim_model\\auxiliary\\7-NLLS\\Data\\"
#define PRINTPATH "C:\\Users\\Joongsan\\source\\repos\\AleP83\\CCE-C\\testbench\\estim_model\\auxiliary\\7-NLLS\\Data\\"
#define N_MODEL 1
...
#define ELEMENT(row,column,row_cnt) row+column*row_cnt
#define REV_ELEMENT(row,column,row_cnt,column_cnt) row+(column_cnt-column-1)*row_cnt
...
real coeff_pop_i[LENGTH_COEFF_POP];
// ----------------------------------------------
...
//const real size_deriv = 1e-5;//This is the original value.
const real size_deriv = 1.25;//This is the test value.
...
void natal_fct(const real logrealgdp_c[],const real temp_c[],const real logrealgdp_w, const small_idx_t t, const real coeff_pop[], real natal_model_aux_Cvect[])
{
	for (small_idx_t i=0;i< 168;i++)
		natal_model_aux_Cvect[ELEMENT(i, t, 168)] = logrealgdp_c[ELEMENT(i,t, 168)]+i+ logrealgdp_w + coeff_pop[i% LENGTH_COEFF_POP];
}

void estimate(const env_t env)
{
	const real* H = env.H;
	const small_idx_t row_cnt = env.length_C_vect;
	const small_idx_t product_cnt = row_cnt * T_BACK_AUX;
	unsigned int allo_size = sizeof(real) * row_cnt * T_BACK_AUX;
	...
	small_idx_t* C_vect = env.C_vect;

	real* oper1 = (real*)calloc(row_cnt, sizeof(real));
	real* oper2 = (real*)calloc(row_cnt, sizeof(real));


	read_binary(env.H, (char*)"hinput", NPOSLAND);
	real* double_country = (real*)malloc(sizeof(real)*NPOSLAND);
	read_binary(double_country, (char*)"cinput", NPOSLAND);
	for (small_idx_t i = 0; i < NPOSLAND; i++)
	{
		C_vect[i] = double_country[i]-1;

	}
	allo_size = sizeof(real) * NPOSLAND * T_BACK_AUX;
	real* l = (real*)malloc(allo_size);
	read_binary(l, (char *)"LINPUT", NPOSLAND * T_BACK_AUX);
	real* realgdp = (real*)malloc(allo_size);
	read_binary(realgdp, (char*)"rinput", NPOSLAND * T_BACK_AUX);
	real* temp = (real*)malloc(allo_size);
	read_binary(temp, (char*)"tinput", NPOSLAND * T_BACK_AUX);
	...
	for (small_idx_t t=0;t<T_BACK_AUX;t++)
	{
		...
		for (i = 0; i < NPOSLAND; i++)
			oper1[C_vect[i]] += H[i] * l[REV_ELEMENT(i, t, NPOSLAND, T_BACK_AUX)] * realgdp[REV_ELEMENT(i, t, NPOSLAND, T_BACK_AUX)];
		for (i = 0; i < NPOSLAND; i++)
			oper2[C_vect[i]]+=H[i] * l[REV_ELEMENT(i, t, NPOSLAND, T_BACK_AUX)];
		for (r = 0; r < row_cnt; r++)
			logrealgdp_c[ELEMENT(r, t, row_cnt)]=log(oper1[r]/ oper2[r]);
		memset(oper1, 0, sizeof(real)* row_cnt);
		for (i = 0; i < NPOSLAND; i++)
			oper1[C_vect[i]] += H[i] * l[REV_ELEMENT(i, t, NPOSLAND, T_BACK_AUX)] * temp[REV_ELEMENT(i, t, NPOSLAND, T_BACK_AUX)];
		for (r = 0; r < row_cnt; r++)
			temp_c[ELEMENT(r, t, row_cnt)] = oper1[r] / oper2[r];
		memset(oper1, 0, sizeof(real) * row_cnt);
		memset(oper2, 0, sizeof(real) * row_cnt);
		for (i = 0; i < NPOSLAND; i++)
		{
			real interme=H[i] * l[REV_ELEMENT(i, t, NPOSLAND, T_BACK_AUX)];
			sum2 += interme;
	...
	double coeff_pop_p[LENGTH_COEFF_POP],coeff_pop_m[LENGTH_COEFF_POP];
	double error_nlls = TOLL_NLLS +1,SSR_f = TOLL_FINAL_SSR + 1;
	unsigned count = 1;
	unsigned char c,t;
	small_idx_t i;
	real* deriv_i = (real*)malloc(sizeof(real) *product_cnt *LENGTH_COEFF_POP);
	real* natal_model_aux_Cvect_p = (real*)malloc(sizeof(real) * product_cnt);
	real* natal_model_aux_Cvect_m = (real*)malloc(sizeof(real) * product_cnt);


	while (error_nlls > TOLL_NLLS && SSR_f > TOLL_FINAL_SSR && count < count_max)
	{
		for (c = 0; c < LENGTH_COEFF_POP; c++)
		{

			memcpy(coeff_pop_p, coeff_pop_i, sizeof(real)*LENGTH_COEFF_POP);
			coeff_pop_p[c] += size_deriv;
			memcpy(coeff_pop_m, coeff_pop_i, sizeof(real)*LENGTH_COEFF_POP);
			coeff_pop_m[c] -= size_deriv;


			for (t = 0; t < T_BACK_AUX; t++)
			{
				natal_fct(logrealgdp_c, temp_c, logrealgdp_w[t], t, coeff_pop_p,natal_model_aux_Cvect_p);
				natal_fct(logrealgdp_c, temp_c, logrealgdp_w[t], t, coeff_pop_m,natal_model_aux_Cvect_m);

			}
			for (i = 0; i < row_cnt; i++)
			{
				for (t = 0; t < T_BACK_AUX; t++)
					deriv_i[ELEMENT(i*T_BACK_AUX+t, c, product_cnt)] = (natal_model_aux_Cvect_p[ELEMENT(i, t, row_cnt)] - natal_model_aux_Cvect_m[ELEMENT(i, t, row_cnt)]) / (2 * size_deriv);
			}
	...
	//print deriv_i
	write_binary(deriv_i, (char*)"coutput", row_cnt*T_BACK_AUX *LENGTH_COEFF_POP);

	//print logrealgdp_c
	//write_binary(logrealgdp_c, (char *)"coutput", row_cnt * T_BACK_AUX);
	//print temp_c
	write_binary(temp_c, (char*)"toutput", row_cnt * T_BACK_AUX);
	//print logrealgdp_w
	write_binary(logrealgdp_w, (char*)"woutput", T_BACK_AUX);
...
void read_binary(const real* X_var, const char variablename[], int dim_n)
{
	FILE* fp;
	char filename[250] = { '\0' };
	memset(filename, '\0', sizeof(filename));
	...
	for (int i = 0; i < dim_n; i++)
	{
		fread((void *)&X_var[i], sizeof(X_var[0]), 1, fp);
	}
...
int main()
{
	...
	small_idx_t* C_vect = (small_idx_t*)malloc(NPOSLAND * sizeof(small_idx_t));
	env.H = H;
	env.C_vect = C_vect;
	env.length_C_vect = 168;
