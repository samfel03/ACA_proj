...
void call_mdiv(const double A[64], const double b[8], double Y[8])
{
    double b_A[64];
    double smax;
    int a;
    ...
    signed char ipiv[8];
    /* RCOND  */
    memcpy(&Y[0], &b[0], 8U * sizeof(double));
...
int main()
{
    ...
    char aux_input_file[] = "BINPUT ";
    input_file[0] = 'A';
    output_file[0] = 'M';
    real aux_matrix[MATRIX_SIZE];
    real short_output[MATRIX_SIZE];

    for (unsigned char i = 0; i < TEST_CNT; i++)
    {
        input_file[6] = i + '0';
        aux_input_file[6] = i + '0';
        output_file[7] = i + '0';
        read_binary(matrix, input_file, DIMENSION(MATRIX_SIZE));
        transpose(matrix, MATRIX_SIZE);
        read_binary(aux_matrix, aux_input_file, MATRIX_SIZE);
        call_mdiv(matrix, aux_matrix, short_output);
        std::cout << char(i + '0');
        write_binary(short_output, output_file, MATRIX_SIZE);
    }
    std::cout << "PART 3 FINISHED SUCCESSFULLY\n";
