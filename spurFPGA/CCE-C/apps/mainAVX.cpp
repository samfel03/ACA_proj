...
#define NPOSLAND 17048    // Number of cells with positive land          17048 !!!!!!!MUST BE A MULTIPLE OF 4 TO WORK WITH AVX!!!!!!
...
//In other words, if one of the errors is very small that is sufficient to terminate the loop even if the other error is very large.
    ...
    while ((error_nlls > TOLL_NLLS) && (SSR_f > TOLL_FINAL_SSR) && (count < count_max))
    {
        ...
        for (int c = 0; c < LENGTH_COEFF_POP; c++)
        {
            ...
            error_nlls += (step * fabs(coeff_pop_f[c]) - scal_nlls * (fabs(env->coeff_pop_i[c]) + lev_nlls));
        }
...
void loop_uhat_AVX(const real tol, const real *trmult_reduced, const real *L, const real L_exp, real *uhat_i, const real *R, const real R_exp) {
    real integral[NPOSLAND];
    double error_arr[4];
    error_arr[0] = tol + 1.;
    int iter = 0;

    while (error_arr[0] >= tol || error_arr[1] >= tol || error_arr[2] >= tol || error_arr[3] >= tol) {

        // Maintains integral[i] calculations as scalar operation
        for (int i = 0; i < NPOSLAND; i++)
            integral[i] = R[i] * pow(uhat_i[i], R_exp);

        // Initialize and reset error and _error
        memset(&error_arr[0], 0, sizeof(error_arr));
        __m256d _error = _mm256_setzero_pd();

        // Packed doubles, so 4 x 64bit can fit into a 256bit YMM
        for (int i = 0; i < NPOSLAND; i += 4) {
            
            // Initialize _rhs vector
            __m256d _rhs = _mm256_setzero_pd();

            
            for (int j = 0; j < NPOSLAND; j++) {
                // rhs += trmult_reduced[IPOSLAND(i, j)] * integral[j];

                __m256d _trmult = _mm256_loadu_pd(&trmult_reduced[IPOSLAND(i, j)]);
                __m256d _integral = _mm256_loadu_pd(&integral[j]); // MIGHT BE THE ERROR HERE (FIXED WITH _mm256_loadu_pd(&integral[j])?)
                __m256d _rhs_inter = _mm256_mul_pd(_trmult, _integral);
                _rhs = _mm256_add_pd(_rhs_inter, _rhs);
            }

            // Initialize _L vector
            __m256d _L = _mm256_loadu_pd(&L[i]);

            // pow(rhs, L_exp);
            double rhs_arr[4];
            // Store 4 _rhs values to rhs_arr for scalar operations
            _mm256_storeu_pd(rhs_arr, _rhs);
            for (int k = 0; k < 4; k++) {
                // Perform 4 scalar power operations for use later
                rhs_arr[k] = pow(rhs_arr[k], L_exp);

            }
            // real uhat_f = L[i] * pow(rhs, L_exp);
            // Load rhs_arr back into _rhs_pow vector form for subsequent multiplication with _L
            __m256d _rhs_pow = _mm256_loadu_pd(rhs_arr);
            __m256d _uhat_f = _mm256_mul_pd(_L, _rhs_pow);

            // real deviation = (uhat_i[i] - uhat_f);
            // Load uhat_i to vector form, multiply by uhat_f, multiply result by itself
            __m256d _uhat_i = _mm256_loadu_pd(&uhat_i[i]);
            __m256d _deviation = _mm256_sub_pd(_uhat_i, _uhat_f);
            __m256d _deviation2 = _mm256_mul_pd(_deviation, _deviation);

            // error += deviation * deviation;
            // Store deviation^2 into _error
            _error = _mm256_add_pd(_error, _deviation2);

            // uhat_i[i] = uhat_f;
            // Update the guess
            _mm256_storeu_pd(&uhat_i[i], _uhat_f);

            // Adding errors for while loop condition
            // Create array of errors, store 4 packed doubles into the array.
            // Not sure if this works properly yet               ----------DEVELOP----------
            _mm256_storeu_pd(error_arr, _error);
            // error = error_arr[0] + error_arr[1] + error_arr[2] + error_arr[3];
        ...
        */
        ...
        while (error_realgdp >= BW_tol_realgdp)
        {
            ...
            while (error_e >= BW_tol_e)
            {
                ...
                loop_uhat_AVX(BW_tol,env->trmult_reduced,BL,env->uhat_f_exp,uhat_i,BR, env->exp_uhatR);
...
/*
                    // NOTE: Scalar is misspelled as scaler
                    __m256d _avgprod_scalar = _mm256_set1_pd(avgprod_scaler); // Create AVX scalar array

                    __m256d _u_i_scaler = _mm256_set1_pd(u_i_scaler);

                    real powout[4];
                    
                    __m256d _ltilde = _mm256_set1_pd(ltilde);

                    real realgdp_w_add[4];

                    for (int i = 0; i < NPOSLAND - 4; i += 4) {


                        // prod[i]=prod[i]*avgprod_scaler;

                        // Load 4 doubles from the array
                        __m256d _prod = _mm256_loadu_pd(&prod[i]);

                        //Multiply _prod with _avgprod_scaler
                        __m256d _result = _mm256_mul_pd(_prod, _avgprod_scalar);

                        // Store the result into _prod
                        _mm256_storeu_pd(&prod[i], _result);
                        ...
                        // Multiply by scaler vector and store into _u_i
                        __m256d _u_i = _mm256_mul_pd(_uhat_i, _u_i_scaler);
                        ...
                        // pow(l_vec[i],env__lambda)
                        for (int j = 0; j < 4; ++j){
                            powout[j] = pow(l_vec[j], env__lambda);
                        }
                        __m256d _powout = _mm256_loadu_pd(powout);

                        // u_i /amen[i]
                        __m256d _amen = _mm256_loadu_pd(&amen[i]);
                        __m256d _divout = _mm256_div_pd(_u_i, _amen);
                        
                        // Final Multiplication
                        __m256d _real_gdp_i = _mm256_mul_pd(_divout, _powout);
                        ...
                        // real_gdp_i*env->H[i]
                        __m256d _H = _mm256_loadu_pd(&env->H[i]);
                        __m256d _multout = _mm256_mul_pd(_real_gdp_i, _H);

                        // l_vec[i]/ltilde
                        __m256d _l_vec = _mm256_loadu_pd(&l_vec[i]);
                        __m256d _divout1 = _mm256_div_pd(_l_vec, _ltilde);

                        // realgdp_w += real_gdp_i*env->H[i]*l_vec[i]/ltilde;
                        __m256d _finalout = _mm256_mul_pd(_multout, _divout);
                        _mm256_storeu_pd(&realgdp_w_add[i], _finalout);
                        for (int x = 0; x < 4; ++x) {
                            realgdp_w += realgdp_w_add[x];
                        }

                        // realgdp[i]= real_gdp_i;
                        _mm256_storeu_pd(&realgdp[i], _real_gdp_i);
                    }
*/
