...
typedef struct {
    ...
    XLoop_uhat_sparse_uio_map maps[ MAX_UIO_MAPS ];
} XLoop_uhat_sparse_uio_info;

/***************** Variable Definitions **************************************/
static XLoop_uhat_sparse_uio_info uio_info;
...
static int uio_info_read_name(XLoop_uhat_sparse_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XLoop_uhat_sparse_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XLoop_uhat_sparse_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XLoop_uhat_sparse_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XLoop_uhat_sparse_Initialize(XLoop_uhat_sparse *InstancePtr, const char* InstanceName) {
	XLoop_uhat_sparse_uio_info *InfoPtr = &uio_info;
